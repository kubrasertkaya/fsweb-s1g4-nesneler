...
function MenuElemaniOlustur2(a, b, c) {
  const yeniUrun = {
    isim: "",
    fiyat: "",
    kategori: "",
...
console.log("deneme2 ", MenuElemaniOlustur2("leb", 18, "gerler"));
...
console.log(MenuElemaniOlustur("Ton Balıklı Makarna", 15, "Makarnalar"));
console.log(MenuElemaniOlustur("Turşu Suyu", 25, "İçecekler"));
...
burger["indirim"] = function (müşteri) {
  if (müşteri === "öğretmen") {
    return burger.fiyat * 0.75;
  } else if (müşteri === "öğrenci") {
    return burger.fiyat * 0.75;
  } else if (müşteri === "diğer") {
    return burger.fiyat * (1 - 0.1);
  } else return "Müşteri indirim kategorisinde değildir.";
};
...
degerlendirmeler[7].geribildirim = reynaBildirim;
...
function DegerledirmeEkle(diziAdi, a, b, c) {
  const newObj = {};
  newObj.isim = a;
  newObj.puan = b;
  newObj.geribildirim = c;
...
function AnahtardanDegerlendirmeAl(diziAdi, a) {
  return (
    diziAdi[a].isim +
    " isimli kişi " +
    diziAdi[a].puan +
    " puan verdi ve şunları yazdı: " +
    diziAdi[a].geribildirim
  );
}
...
function SonDegerlendirmeyiAl(diziAdi) {
  return (
    diziAdi[diziAdi.length - 1].isim +
    " isimli kişi " +
    diziAdi[diziAdi.length - 1].puan +
    " puan verdi ve şunları yazdı: " +
    diziAdi[diziAdi.length - 1].geribildirim
  );
}
